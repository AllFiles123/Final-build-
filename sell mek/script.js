...
function setupHeroVisualLoop(){
    ...
    if(!hero || !slider || slides.length < 2) return;

    /* Keep the dashboard in its existing stage when present. */
    if(dashboard && dashboardStage && !dashboardStage.contains(dashboard)){
...
/* =====================================================
   HOME — OUR POPULAR SERVICES AUTO SLIDER
===================================================== */
function setupPopularServicesSlider(){
    const viewport=document.querySelector('.popular-services-viewport');
    const track=document.querySelector('.popular-services-track');
    if(!viewport||!track) return;
    const cards=Array.from(track.querySelectorAll('.popular-service-card'));
...
function setupAboutMarketingSlider(){
    const track=document.querySelector('.about-marketing-track');
...
if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',()=>{setupPopularServicesSlider();setupAboutMarketingSlider();});
}else{
    setupPopularServicesSlider();setupAboutMarketingSlider();
}
